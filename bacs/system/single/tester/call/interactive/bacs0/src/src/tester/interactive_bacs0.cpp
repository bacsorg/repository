...
bool tester::test(const problem::single::settings::ProcessSettings &settings,
...

  {
    ...
    pimpl->container->filesystem().setOwnerId(interactor_output,
                                              INTERACTOR_OWNER_ID);
  ...
  const bool interactor_execution_success = process::parse_result(
      process_group_result, interactor_result, *result.mutable_judge()
                                                    ->mutable_utilities()
                                                    ->mutable_interactor()
                                                    ->mutable_execution());
...

  {
    ...
    const bool pipectl_execution_success =
        process::parse_result(process_group_result, pipectl_result,
