...
class tester::impl {
 public:
  explicit impl(const ContainerPointer &container_)
      : container(container_), checker_(container_) {}
...
bool tester::build(const bacs::process::Buildable &solution,
  ...
  pimpl->solution = pimpl->builder->build(
      pimpl->container, SOLUTION_OWNER_ID, solution.source(),
...
bool tester::test(const problem::single::settings::ProcessSettings &settings,
  ...
  pimpl->container->filesystem().setOwnerId(current_path, JUDGE_OWNER_ID);
  pimpl->container->filesystem().setMode(current_path, 0500);

  pimpl->container->filesystem().setOwnerId(logging_path, JUDGE_OWNER_ID);
  ...
  pimpl->container->filesystem().setOwnerId(judge_to_solution_log_path,
                                            JUDGE_OWNER_ID);
  ...
  pimpl->container->filesystem().setOwnerId(solution_to_judge_log_path,
                                            JUDGE_OWNER_ID);
  ...
  const ProcessPointer solution =
      pimpl->solution->create(process_group, settings.execution().argument());
  ...
  const ProcessPointer broker = process_group->createProcess(
      "yandex_contest_invoker_flowctl_interactive_simple_broker");
  broker->setName("broker");
  broker->setArguments(broker->executable(), "--notifier", "3",
                       "--interactor-source", "4", "--interactor-sink", "5",
                       "--solution-source", "6", "--solution-sink", "7",
  ...
  detail::file::touch(
      pimpl->container->filesystem().keepInRoot(interactor_output));
  ...
  const boost::filesystem::path interactor_custom_message =
      current_path / "custom_message";
  env["JUDGE_INTERACTOR_CUSTOM_MESSAGE"] = interactor_custom_message.string();
  detail::file::touch(
      pimpl->container->filesystem().keepInRoot(interactor_custom_message));
  pimpl->container->filesystem().setOwnerId(interactor_custom_message,
                                            JUDGE_OWNER_ID);
  ...
  const bool execution_success = process::parse_result(
      process_group_result, solution_result, *result.mutable_execution());

  const auto read_file = [&](problem::single::result::File &rfile,
                             const problem::single::settings::File &file,
                             const boost::filesystem::path &path) {
    rfile.set_id(file.id());
    *rfile.mutable_data() = bacs::system::file::read(path, file.receive());
  };
  ...
  const bool interactor_execution_success = process::parse_result(
      process_group_result, interactor_result,
  ...
  const bool broker_execution_success = process::parse_result(
      process_group_result, broker_result,
      ...
      switch (broker_execution.exit_status()) {
        case Broker::SOLUTION_TERMINATION_REAL_TIME_LIMIT_EXCEEDED:
          if (!interactor_execution.has_exit_status()) {
            judge.set_status(problem::single::result::Judge::
                                 TERMINATION_REAL_TIME_LIMIT_EXCEEDED);
          } else if (return_cast(interactor_execution.exit_status()) ==
        ...
        broker_ok_:
          ...
          switch (judge.status()) {
            ...
            case problem::single::result::Judge::CUSTOM_FAILURE:
              judge.set_message(pimpl->read_first(interactor_custom_message,
                                                  MAX_MESSAGE_SIZE));
              break;
          }
          break;
        case Broker::SOLUTION_OUTPUT_LIMIT_EXCEEDED:
          judge.set_status(
              problem::single::result::Judge::OUTPUT_LIMIT_EXCEEDED);
          break;
