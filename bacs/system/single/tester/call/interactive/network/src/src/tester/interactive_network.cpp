...
class tester::impl : public detail::tester {
 public:
  explicit impl(const ContainerPointer &container)
      : tester(container, "/testing"), checker_(container) {}
...
bool tester::test(const problem::single::settings::ProcessSettings &settings,
  ...
  const ProcessPointer process = pimpl->solution->create(
      pimpl->process_group(), settings.execution().argument());
  ...
  interactor->setStream(
      0, pimpl->add_notifier(NotificationStream::Protocol::PLAIN_TEXT));

  // note: ignore current_path from settings.execution()
  process->setCurrentPath(testing_path);

  pimpl->set_test_files(process, settings, test_, testing_path,
                        SOLUTION_OWNER_ID);
  ...
  pimpl->copy_test_file(test_, "in", test_in, INTERACTOR_OWNER_ID, 0400);
  ...
  if (data_set.find("out") != data_set.end()) {
    ...
    pimpl->copy_test_file(test_, "out", test_hint, INTERACTOR_OWNER_ID, 0400);
  ...
  pimpl->touch_test_file(interactor_output, INTERACTOR_OWNER_ID, 0600);
  ...
  pimpl->touch_test_file(interactor_custom_message, INTERACTOR_OWNER_ID, 0600);
  ...
  pimpl->touch_test_file(interactor_log_path, INTERACTOR_OWNER_ID, 0600);
  ...
  const bool execution_success =
      pimpl->parse_result(process, *result.mutable_execution());
  const bool interactor_execution_success =
      pimpl->parse_result(interactor, *result.mutable_judge()
                                           ->mutable_utilities()
                                           ->mutable_interactor()
                                           ->mutable_execution());
  ...
  judge.mutable_utilities()->mutable_interactor()->set_output(
      pimpl->read_first(interactor_log_path, MAX_MESSAGE_SIZE));
  ...
  switch (judge.status()) {
    ...
    case problem::single::result::Judge::CUSTOM_FAILURE:
      judge.set_message(
          pimpl->read_first(interactor_custom_message, MAX_MESSAGE_SIZE));
      break;
  }
