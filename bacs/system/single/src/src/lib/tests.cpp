   ...
   private:
    int flags(const problem::single::testing::WildcardQuery &query) {
      ...
      for (const int flag : query.flag()) {
        switch (
            static_cast<problem::single::testing::WildcardQuery::Flag>(flag)) {
   ...
   public:
    bool match(const std::string &test_id) const override {
      for (const std::unique_ptr<impl> &imp : *this)
        if (imp->match(test_id)) return true;
      return false;
    }
  };

 private:
  static std::unique_ptr<impl> impl_(const google::protobuf::RepeatedPtrField<
      problem::single::testing::TestQuery> &test_query) {
    auto tmp = std::make_unique<any_of>();
...
test::test(tests &tests_, const std::string &test_id)
    : m_tests(&tests_), m_test_id(test_id) {}

void test::copy(const std::string &data_id,
