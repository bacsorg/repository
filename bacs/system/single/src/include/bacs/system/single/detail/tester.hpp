...
class tester : private tester_mkdir_hook {
 ...
 public:
  ...
  boost::filesystem::path create_directory(
      ...
      const unistd::access::Id &owner_id, const mode_t mode = 0700) {
    try {
  ...
  void set_test_files(
      ...
      const problem::single::settings::ProcessSettings &settings,
      const test &test_, const boost::filesystem::path &destination,
      const unistd::access::Id &owner_id, const mode_t mask = 0777) {
    try {
  ...
  void copy_test_file(const test &test_, const std::string &data_id,
                      const boost::filesystem::path &destination,
                      const unistd::access::Id &owner_id, const mode_t mode) {
    try {
      test_.copy(data_id, m_container->filesystem().keepInRoot(destination));
  ...
  void touch_test_file(const boost::filesystem::path &destination,
                       const unistd::access::Id &owner_id, const mode_t mode) {
    try {
      detail::file::touch(m_container->filesystem().keepInRoot(destination));
      m_container->filesystem().setOwnerId(destination, owner_id);
      m_container->filesystem().setMode(destination, mode);
    } catch (std::exception &) {
  ...
  std::string read_last(const boost::filesystem::path &path,
    ...
    try {
      ...
      BOOST_THROW_EXCEPTION(read_last_error()
                            << bunsan::enable_nested_current());
    }
  }

  struct send_file_error : virtual error {};
  void send_file(bacs::problem::single::result::TestResult &result,
    ...
    try {
      ...
      *file.mutable_data() = bacs::system::file::read(
      ...
      BOOST_THROW_EXCEPTION(send_file_error()
                            << bunsan::enable_nested_current());
    }
  }

  struct send_file_if_requested_error : virtual error {};
  void send_file_if_requested(bacs::problem::single::result::TestResult &result,
  ...
  void send_test_files(bacs::problem::single::result::TestResult &result) {
    try {
  ...
  void redirect(const ProcessPointer &from, const int from_fd,
    ...
    try {
      ...
      BOOST_THROW_EXCEPTION(redirect_error()
                            << bunsan::enable_nested_current());
    }
  }
  ...
  bool fill_status(problem::single::result::TestResult &result) {
    result.set_status(ok(result) ? problem::single::result::TestResult::OK
 ...
 private:
  ...
  file_map m_test_files, m_solution_files, m_container_solution_files;
};
